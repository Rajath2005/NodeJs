...
// Pure React tokenizer - renders JSX spans, no dangerouslySetInnerHTML
function tokenizeLine(line) {
  ...
  const patterns = [
    ...
    { regex: /\.\b(log|join|resolve|extname|basename|dirname|readFileSync|readdirSync|statSync|isFile|execSync|arch|platform|networkInterfaces|cpus|freemem|totalmem|uptime|userInfo|cwd|addition|subtraction|multiplication|division)\b/, className: "tok-method" },
...
export default function LearningHub({ files }) {
  const [activeFile, setActiveFile] = useState(files.length > 0 ? files[0] : null);
      ...
      <div className="shell">
        ...
        <aside className={`sidebar ${menuOpen ? "sidebar--open" : ""}`}>
          ...
          <nav className="sidebar__list">
            {files.length === 0 && <p className="sidebar__hint">No files found.</p>}
            {files.map((f) => (
              <button
                key={f.name}
        ...
        <main className="viewer">
          {activeFile ? (
            ...
            <div className="viewer__empty">Select a file to begin learning</div>
          )}
        </main>
      </div>
    </>
